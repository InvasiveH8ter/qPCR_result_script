...
df_3[is.na(df_3)] <- 0
colnames(df_3) <- c('Sample.Name', 'qPCR.Replicates', 'Amps','Copy.mean', 
...
Target.Detection = ifelse(test= my_results$adj_quant > 10 & my_results$Amps > 1,
                          yes = "Yes",
                          no = "No")
Sample.Inhibition = ifelse(test= my_results$Ct.Delay > 3,
                           yes = "Yes",
                           no = "No")
